  ...
  actions: {
    ...
    setupUtteranceEvents(utterance, content) {
      ...
      if (state.browserType === 'safari') {
        ...
        utterance._safariTimerId = actions.setupSafariHighlighting(content);
      } else {
        utterance.onboundary = event => {
        ...
        newUtterance.onboundary = event => {
          ...
          if (mainElement) {
            ...
            if (state.nodePositions.size === 0) {
              actions.buildNodePositionsMap(mainElement, excludeClassesStr.split(/\s+/));
            }

            // Handle the boundary event with the full content context
            actions.handleBoundaryEvent(adjustedEvent, state.pausedAt.fullText || state.pausedAt.text);
          }
        };
    ...
    handleBoundaryEvent(event, content) {
      ...
      if (event.charIndex === undefined || event.charLength === undefined || event.charIndex + event.charLength > content.length) {
        console.warn('Invalid boundary event or position out of content bounds');
        return;
      }
      try {
        if (state.selectedTextRange?.hasSelection) {
          ...
          if (state.nodePositions.size === 0) {
            ...
            actions.buildNodePositionsMap(mainElement, excludeClasses);
          }
          ...
          for (let i = 0; i < positions.length; i++) {
            ...
            // Exact match
            if (pos <= charPosition && charPosition < pos + nodeData.length) {
