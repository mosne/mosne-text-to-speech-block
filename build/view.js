...
const {
  ...
  state: {
    isPlaying: false,
    currentVoice: null,
    preferredVoice: window.localStorage.getItem('mosne-tts-lang-' + document.documentElement.lang) || null,
    utterance: null,
    voices: [],
    currentSpeed: window.localStorage.getItem('mosne-tts-speed-' + document.documentElement.lang) || 1,
    currentPitch: window.localStorage.getItem('mosne-tts-pitch-' + document.documentElement.lang) || 1,
    selectedTextRange: {
      hasSelection: false
    },
    currentChunk: 0,
    ...
    createUtterance() {
      ...
      newUtterance.onboundary = function (event) {
        ...
        try {
          ...
          if (state.selectedTextRange && state.selectedTextRange.hasSelection) {
            ...
            if (selection && selection.rangeCount > 0) {
              ...
              selectionWrapper.className = 'mosne-tts-highlighted-word';
              selectionWrapper.style.setProperty('--mosne-tts-highlight-bg', highlightBackground);
              selectionWrapper.style.setProperty('--mosne-tts-highlight-color', highlightColor);
      ...
      newUtterance.onend = function () {
        ...
        if (state.currentChunk < state.textChunks.length - 1 && state.isPlaying) {
          // Move to next chunk
          state.currentChunk++;
